...
/**
 * 校验 PUT /api/config 的补丁字段类型；返回错误消息数组（空 = 通过）。
 * 与 /api/agents 路由的校验强度对齐，防止非法值写盘后下次启动崩溃。
 */
function validateConfigPatch(patch: Record<string, unknown>): string[] {
  const errors: string[] = [];
  if ("allowFrom" in patch) {
    if (!Array.isArray(patch.allowFrom) || !patch.allowFrom.every((v) => typeof v === "string")) {
      errors.push("allowFrom 必须是字符串数组");
    }
  }
  if ("defaultAgent" in patch) {
    if (typeof patch.defaultAgent !== "string" || !patch.defaultAgent.trim()) {
      errors.push("defaultAgent 必须是非空字符串");
    }
  }
  if ("agents" in patch) {
    ...
    if (typeof agents !== "object" || agents === null || Array.isArray(agents)) {
      ...
      for (const [id, a] of Object.entries(agents as Record<string, unknown>)) {
        if (typeof a !== "object" || a === null || Array.isArray(a)) {
          errors.push(`agents.${id} 必须是对象`);
          continue;
        }
        const cfg = a as Record<string, unknown>;
        if (typeof cfg.command !== "string" || !cfg.command.trim()) {
          errors.push(`agents.${id}.command 必须是非空字符串`);
        }
        if ("args" in cfg && !Array.isArray(cfg.args)) {
          errors.push(`agents.${id}.args 必须是数组`);
        }
        if ("cwd" in cfg && typeof cfg.cwd !== "string") {
          errors.push(`agents.${id}.cwd 必须是字符串`);
        }
        if ("notifyPolicy" in cfg && parseNotifyPolicy(cfg.notifyPolicy) === undefined) {
          errors.push(`agents.${id}.notifyPolicy 必须是 "none" | "own" | "all"`);
        }
      }
    }
  }
  if ("autoApprove" in patch && typeof patch.autoApprove !== "boolean") {
    errors.push("autoApprove 必须是布尔值");
  }
  if ("webPort" in patch) {
    const p = patch.webPort;
    if (typeof p !== "number" || !Number.isInteger(p) || p < 1 || p > 65535) {
      errors.push("webPort 必须是 1-65535 的整数");
    }
  }
  if ("pluginsFile" in patch) {
    if (typeof patch.pluginsFile !== "string" || !patch.pluginsFile.trim()) {
      errors.push("pluginsFile 必须是非空字符串");
    }
  }
  for (const key of ["streamFlushMinChars", "streamFlushIdleMs"] as const) {
    if (key in patch) {
      const v = patch[key];
...
async function handleApi(req: Request, pathname: string, deps: ApiDeps): Promise<Response> {
  ...
  if (pathname === "/api/config" && req.method === "PUT") {
    ...
    if (errors.length > 0) {
      return jsonError(400, `配置校验失败: ${errors.join("; ")}`);
    }
